...
### Stale bd issues (found via `beads-recall` dispatch, added during plan-phase 18)
- **D-09:** `18-BEADS-RECALL.md` (generated by this plan-phase run) surfaced 4 open bd issues that
  map 1:1 to TRUTH-01..04, all already fixed and verified in Phase 17, never auto-closed at
  `execute:wave:post`: `gsd-beads-he1` (TRUTH-03), `gsd-beads-bzl` (TRUTH-04, P1),
  `gsd-beads-v43` (TRUTH-01, P1), `gsd-beads-t7a` (TRUTH-02, P3). Add a task that runs
  `sync.py reconcile-stale-closed` scoped to Phase 17, confirms all 4 close, and verifies via
  `bd show` that each closed issue is genuinely one of TRUTH-01..04 (not a misidentification) —
  same mechanism already used once for 4 stale Phase 14 issues (`gsd-beads-bu0.3`–`.6`). Motivation
  beyond hygiene: `beads.ship_gate` blocks `ship:pre` when `BEADS.md` reports `blocking_open > 0`
  — 3 of these 4 are P1, so left open they risk blocking Phase 18's own ship step over work that
  is already done.
  — **Reversibility:** reversible — `bd close` is not a destructive git operation; issues remain
  in `bd`'s history, closed.
...
- `.planning/phases/18-address-tech-debt-patch-check-doc-accuracy-changelog/18-BEADS-RECALL.md` —
  generated live during this plan-phase run; source of D-09 (4 stale bd issues) and confirms
  D-08's revised scope (both check-patch calls returned `⚠` for both ship-md and execute-plan).
- `plugins/beads-lifecycle/.gsd/capabilities/beads/GSD-CORE-PATCH.md` §"Patch Content (verbatim)"
  (both Patch 1 and Patch 2 sections) — the exact insertion anchors and patch text D-08's reapply
  task must use.
