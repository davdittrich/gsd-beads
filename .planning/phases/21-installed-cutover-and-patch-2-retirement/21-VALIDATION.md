...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 21-01-01 | 01 | 1 | CUT-01 | Global-install proof cannot be satisfied by a project overlay; malformed or unavailable resolver inputs stop non-zero without fallback. | Existing exact-argv spy plus fresh live public integration | Self-contained bounded `/dev/shm` transaction proving exact SHA, natural identities/live row, three-tree parity, global-only registry selection, authored five-field positive, four isolated negatives, mutation absence, and cleanup; then the existing exact `bd show` argv spy | ✅ inline / existing | ⬜ pending |
| 21-01-02 | 01 | 2 | CUT-02 | Patch 2 is deleted only after the pre-removal gate; Patch 1 remains byte-identical and independently executable. | TDD retirement regression plus fresh live public integration | Execution-local residue/Patch-1/rollback transaction; repeated public positive/four-negative transaction and argv spy; Python compile plus full capability suite | ✅ inline / existing | ⬜ pending |
