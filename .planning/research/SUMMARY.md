...
resolver as `python3 -c <stdlib locator> {{id}}`, with a 10,000 ms timeout. The
locator derives the global installed capability root from `GSD_HOME` or
`Path.home()`, then uses `os.execv` to invoke its adapter script with the id as
a separate argv item. The adapter alone translates live Beads issue data into
the one JSON object gsd-core requires; gsd-core continues to own exact task
lookup, resolver selection, timeouts, and hard failure. This preserves the
overlay model, live Beads authority, and the no-new-dependency constraint.
**[98/100]**
...
The locator decision is resolved by direct non-root-cwd proof; it is not an
upstream blocker. The remaining risk is operational: the active registry,
installed copy, database discovery, and both runtime homes must still be proven
at cutover. Retire Patch 2 only after an installed-bundle public-command proof
succeeds, all negative paths hard-halt, source and installed copies are
...
`beads` tracker prefix, the approved `python3 -c` bootstrap, an isolated
`{{id}}` argv element, and a 10,000 ms timeout. Its stdlib program imports
`os`, `sys`, and `Path`; derives `Path(os.environ.get("GSD_HOME") or
Path.home())`; joins the `.gsd`, `capabilities`, `beads`, `scripts`, and
`sync.py` components; then calls `os.execv` with Python, that installed script,
`resolve-task-content`, and `sys.argv[1]`. The existing adapter is the approved
...
packaging, or release surface. **[98/100]**
...
**Rejected mechanism:** A PATH shim adds a wrapper plus install, update,
uninstall, precedence, and collision lifecycle for no capability gain. The
approved locator is cwd-independent, needs no new PATH state, and reaches the
globally installed copy directly. **[100/100]**
...
behavior. The direct non-root locator proof establishes the chosen path; the
installed public command and installed-copy parity remain execution gates.
**[98/100]**
...
identity and launches `python3 -c <locator> {{id}}`; the locator uses
`GSD_HOME`/`Path.home()` plus `os.execv` to start the installed adapter with the
id as a separate argv element. Its proposed resolver verb executes fixed-argv
`bd show <id> --json`, validates one issue, and prints the mapped JSON object to
stdout only. `<beads-id>` remains the identity used by lifecycle status,
dependencies, close-wave, and reconciliation. The patch documentation should
retain Patch 1 only after all Patch-2 marker, checker, route, and documentation
references are removed together. **[98/100]**
...
**Delivers:** One resolver declaration using the approved `python3 -c` locator;
one narrow stdlib adapter resolver command; validator and public-boundary tests
for output mapping, malformed stdout, unavailable/non-zero `bd`, timeout,
prefix collision, and no stdout pollution.
...
**Research flag:** Standard core patterns and the installed-copy locator are
directly characterized; skip generic framework research. Observe the live
criteria field before implementation. **[98/100]**
...
**Research flag:** Requires fresh operational proof immediately before execution:
exercise the fixed locator, verify active registry uniqueness and installed-copy
parity, then prove the public command. This is release evidence, not an
unresolved locator design. **[98/100]**
...
- **Phase 3:** perform fresh installed-runtime invocation, registry, database,
  and byte-parity proof immediately before retirement; this validates the chosen
  mechanism rather than reopens its design.

Phases with standard patterns (skip broad research-phase):

- **Phase 1:** gsd-core's 1.12 resolver contract, exact identity lookup,
  validator bounds, hard-failure taxonomy, and fixed stdlib locator have direct
  installed-runtime evidence. Limit research to the live criteria field.

## Confidence Assessment

| Area | Confidence | Notes |
|------|------------|-------|
| Stack | HIGH | Native contracts and the fixed stdlib locator have direct runtime evidence; live resolution remains a release proof. |
...
- Live non-root-cwd locator proof on 2026-08-30 — the `GSD_HOME`/`Path.home()`
  bootstrap resolved the installed adapter and found a file; the installed script
  is mode 0644 and no equivalent resolver executable is on `PATH`.
